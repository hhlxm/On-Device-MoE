...
def apply_sparse_silu_mlp(
    ...
    # 只在 GPU 0 上显示进度条
    show_progress = torch.cuda.is_available() and torch.cuda.current_device() == 0
    
    layers_iter = tqdm(model.model.layers, desc="Applying sparse MLP", disable=not show_progress)


    for layer in layers_iter:
        if SparseMLP == SparseMixtralSparseMoeBlock:
            original_mlp = layer.block_sparse_moe
            original_device = next(original_mlp.parameters()).device
            # 新模块初始化时直接放到正确设备上
            new_mlp = SparseMLP(config, use_sparse_regularization=use_sparse_regularization).to(original_device)
            for idx, expert in enumerate(new_mlp.experts):
                expert.w1 = original_mlp.experts[idx].w1
                expert.w3 = original_mlp.experts[idx].w3
                expert.w2 = original_mlp.experts[idx].w2
                print(f"expert device{expert.w1.weight.device} original device: {original_mlp.experts[idx].w1.weight.device}")
            #门控拷贝
...
def plot_histogram(
    ...
    if is_mainprocess():
        torch.save(bin_edges, f"{activation_histogram_dir}/bin_edges_{layer_index}_{expert_index}.pt")
        torch.save(histogram_counts, f"{activation_histogram_dir}/histogram_counts_{layer_index}_{expert_index}.pt")
...
class MixtralSparseSiluMLP(MixtralBLockSparseTop2MLP):
    ...
    def forward(
        ...
        """
        If kill_sparse_swish_outputs is set to False, this layer functions exactly like a normal MLP layer.
        w1:gate w3:up_proj w2:down_proj
        """
        if sp_mask != None:  # When sparse mask is given
            return self.w2(
                self.sparse_act_fn(self.w1(x) * sp_mask) * self.w3(x)
            ) 
        ...
        if self.is_profile:
            if x.shape[1] == 1:
                if self.sp_method == 1:
                    ...
                    dead_neurons = post_act.abs() <= self.dead_threshold
                    post_act[dead_neurons] = 0
                    return self.w2(post_act * self.w3(x))
            else:
                post_act = self.act_fn(self.w1(x))
                dead_neurons = post_act.abs() <= self.dead_threshold
                post_act[dead_neurons] = 0
        ...
        # up output
            ...
            out = self.w2(self.act_fn(self.w1(x)) * post_act)
            # out = self.w2(post_act * self.w3(x))
            if self.use_sparse_regularization:
                if self.regularization_type == "L1 regularization":
                    self.activation_norm = torch.abs(post_act)[
...
class SparseMixtralSparseMoeBlock(MixtralSparseMoeBlock):

    def __init__(self, config, *args, **kwargs):
        super().__init__(config)
        self.num_local_experts = config.num_local_experts
        self.experts = nn.ModuleList([MixtralSparseSiluMLP(config, *args, **kwargs) for _ in range(self.num_local_experts)])
        # 为每个 expert 初始化稀疏预测器 sp_mlp
        # self.sp_mlps = nn.ModuleList([
        #     low_rank_approximation(
        #         expert.w1,
        #         act_func=expert.sparse_act_fn,
        #         init_svd=kwargs.pop("init_svd", True),
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for expert_idx in range(self.num_experts):
            ...
            # total_distill_loss += loss_func(sp_mask, gating_output)
        ...
        # self.distill_loss = total_distill_loss / self.num_experts  # 平均蒸馏损失

        final_hidden_states = final_hidden_states.reshape(batch_size, sequence_length, hidden_dim)
        return final_hidden_states, router_logits


# Mixtral 稀疏DecoderLayer
class SparseMixtralDecoderLayer(MixtralDecoderLayer):
    def __init__(self, config, layer_idx, decoder_layer, init_svd=True, *args, **kwargs):
        assert isinstance(decoder_layer.block_sparse_moe, SparseMixtralSparseMoeBlock), f"{type(decoder_layer.block_sparse_moe)} should be SparseMixtralSparseMoeBlock."
        super().__init__(config, layer_idx)
        self.hidden_size = decoder_layer.hidden_size
        self.block_sparse_moe = decoder_layer.block_sparse_moe
        self.self_attn = decoder_layer.self_attn
        self.input_layernorm = decoder_layer.input_layernorm
        self.post_attention_layernorm = decoder_layer.post_attention_layernorm
        # 稀疏预测器相关
    ...
    def forward(
        ...
        **kwargs,
        ...
        self.distill_loss = self.block_sparse_moe.distill_loss  # 获取 MoE 层的平均蒸馏损失
...
class SparseMixtralConfig(MixtralConfig):
    model_type = "sparse_mixtral"

    def __init__(self, **kwargs):
        super().__init__(**kwargs)

# Mixtral 稀疏CausalLM
class SparseMixtralForCausalLM(MixtralForCausalLM):
    config_class = SparseMixtralConfig
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        if config.use_sparse_model:
            ...
            if config.thresholds is not None:
                for idx, m in enumerate(self.model.layers):
                    if isinstance(m.block_sparse_moe, SparseMixtralSparseMoeBlock):
                        for expert_idx, expert in enumerate(m.block_sparse_moe.experts):
                            expert.dead_threshold = config.thresholds[idx][expert_idx]
                            expert.sparse_act_fn.set_new_threshold(expert.dead_threshold)
                            expert.kill_sparse_swish_outputs = True
                            expert.use_relu = config.use_relu
        if config.use_sparse_predictor:
            self.apply_sparse_predictor(init_svd=config.init_svd)
    def apply_sparse_mlp(self):
        apply_sparse_silu_mlp(self, config=self.config, use_sparse_regularization=self.config.use_sparse_regularization)
    def apply_sparse_predictor(self, init_svd=True):
        apply_sparse_decoder_layer(self, config=self.config, init_svd=init_svd)
