...
###
 ...
 # @Date: 2025-05-23 23:32:49
 # @LastEditors: hhlxm 578723542@qq.com
 # @LastEditTime: 2025-05-24 15:32:39
 # @FilePath: /lxm/Analyse/scripts/ablation/pre_ratio/run_phi.sh
...
for dataset in "${DATASETS[@]}"; do
    for cache_ratio in "${CACHE_RATIOS[@]}"; do
        for pre_ratio in "${PRE_RATIOS[@]}"; do
          ...
          # 生成临时配置文件并运行
          config_file="./phi_${dataset}_${cache_ratio}_${pre_ratio}.yaml"
          generate_config "$dataset" "$cache_ratio" "$pre_ratio"> "$config_file"
          sbatch_script="./sbatch_phi_${dataset}_${cache_ratio}_${pre_ratio}.sh"
          cat > "$sbatch_script" << EOF
#!/bin/bash
#SBATCH -J phi_${dataset}_${cache_ratio}_${pre_ratio}
