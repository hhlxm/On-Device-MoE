    ...
    for layer_idx, layer in enumerate(moe_layers):
        ...
        for batch_idx in batch_indices:
            if layer in frequencies[batch_idx]:
                # 提取当前层的所有 routed 数据并展开为列表
                routed_data = frequencies[batch_idx][layer]["routed"]
                layer_frequencies.extend(
                    [routed_data[key] for key in routed_data.keys()]
                )
        # print(layer_frequencies)
        # 确保有数据
        if layer_frequencies:  
            # 对所有 batch 的该层数据求平均
            activation_matrix[layer_idx] = np.mean(layer_frequencies, axis=0)
