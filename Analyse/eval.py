...
def main():
    ...
    cache_dir = os.path.join(output_config["dir"], model_short_name,dataset_name, f"Cache_hit_ratio_{model_config.get('cache_ratio', 0.5)}")
    ...
    try:
        cache_result = cache_process(
            model, tokenizer, texts, generation_config["max_new_tokens"], model_config["device"],generation_config.get("hot", True)
        )
    except Exception as e:
    ...
    try:
        ...
        for cache_type in cache_result:
            json_compatible_stats[cache_type] = {
                'total_hits': {str(k): v for k, v in cache_result[cache_type]['total_hits'].items()},
                'total_requests': {str(k): v for k, v in cache_result[cache_type]['total_requests'].items()},
                'hit_rates': {str(k): v for k, v in cache_result[cache_type]['hit_rates'].items()},
