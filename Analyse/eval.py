...
from utli import draw, draw_batch, plot_cosine_similarity_avg_cross_batch, plot_layer_token_similarity_bar
...
def process_texts(model, tokenizer, texts, max_new_tokens, device="cuda"):
    ...
    model.reset_all_expert_hit_rate()
    model.reset_all_token_frequency()
    return frequencies1, frequencies3, frequencies4
...
def main():
    config_path = sys.argv[1] if len(sys.argv) > 1 else "/home/pairshoe/lxm/Analyse/config/config.yaml"
    ...
    # 处理文本并收集频率数据
    try:
        frequencies1, frequencies3, frequencies4 = process_texts(
