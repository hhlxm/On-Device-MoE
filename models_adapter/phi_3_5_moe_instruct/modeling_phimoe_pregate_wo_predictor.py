...
class PhiMoESparseMoeBlock(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.Tensor, gate_hidden_states=None
    ) -> torch.Tensor:
        """ """
        ...
        if self.training and self.input_jitter_noise > 0:
            ...
            if gate_hidden_states is not None:
                gate_hidden_states *= torch.empty_like(gate_hidden_states).uniform_(
                    1.0 - self.input_jitter_noise, 1.0 + self.input_jitter_noise
                )
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, router_logits = self.block_sparse_moe(
            hidden_states_input, gate_hidden_states=prev_attn_hidden_states
        )
        hidden_states = residual + hidden_states
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    use_cache=use_cache,
                    prev_attn_hidden_states=pre_hidden[i - self.pre_ahead]
                    if i >= self.pre_ahead
                    else None,
                )
        ...
        if not self.training:
            ...
            for i in range(0, len(self.layers)):  # 从第1层MoE开始，第一层应该是100%相同
                if (
                    all_topk_idx[i] is not None  # 当前层是 MoE 层，有真实 topk_idx
                    and all_pre_topk_idx[i] is not None  # 有预测的 pre_topk_idx
                ):
                    ...
                    true_topk_onehot = torch.zeros(
                        ...
                        device=true_topk_idx.device,
                    )
                    pred_topk_onehot = torch.zeros_like(true_topk_onehot)
                    true_topk_idx = true_topk_idx.view(
                        batch_size, seq_length, self.config.num_experts_per_tok
                    )
                    pred_topk_idx = pred_topk_idx.view(
                        batch_size, seq_length, self.config.num_experts_per_tok
                    )
                    ...
                    # 计算 IoU
                    intersection = (pred_topk_onehot * true_topk_onehot).sum(
                        dim=-1
                    )  # [batch_size, seq_len]
                    correct = (
                        intersection / self.config.num_experts_per_tok
                    )  # [batch_size, seq_len]
                    masked_accuracy = (
                        correct.to(attention_mask.device) * attention_mask
                    ).sum() / attention_mask.sum()
                    union = (
                        pred_topk_onehot.sum(dim=-1)
                        + true_topk_onehot.sum(dim=-1)
                        - intersection
                    )
                    iou = intersection / (union + 1e-8)  # [batch_size, seq_len]
                    masked_iou = (
                        iou.to(attention_mask.device) * attention_mask
                    ).sum() / attention_mask.sum()
