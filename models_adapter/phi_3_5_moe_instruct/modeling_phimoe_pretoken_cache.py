...
class CacheEntry:
    def __init__(self, expert_id, frequency=0, last_access_time=None):
        ...
        self.last_access_time = (
            last_access_time if last_access_time is not None else time.time()
        )
...
class CacheSimulator:
    def __init__(self, capacity, strategy="lru", max_expert_id=64):
        ...
        if self.strategy == "lru" or self.strategy == "fifo":
            ...
            raise ValueError(
                f"Unknown cache strategy: {strategy}. Supported: 'lru', 'fifo', 'lfu'"
            )
    ...
    def _evict_lru(self):
        if not self.cache_store:
            return None
        return self.cache_store.popitem(last=False)[
            0
        ...
        if self.strategy == "lru":
            ...
            if expert_id in self.cache_store:
                ...
                new_entry = CacheEntry(
                    expert_id, frequency=1, last_access_time=current_time
                )
                self.cache_store[expert_id] = new_entry
                heapq.heappush(
                    self.frequency_heap,
                    (new_entry.frequency, new_entry.last_access_time, expert_id),
                )
    ...
    def access(self, expert_id):
        ...
        if self.strategy == "lru":
            if expert_id in self.cache_store:
                ...
                if len(self.cache_store) >= self.capacity:
                    self._evict_lru()
                self.cache_store[expert_id] = (
                    None  # Value doesn't matter for LRU/FIFO here
                )
            ...
            if expert_id in self.cache_store:
                ...
                heapq.heappush(
                    self.frequency_heap,
                    (entry.frequency, entry.last_access_time, expert_id),
                )
...
class Predictor(nn.Module):
    def __init__(self, config):
        ...
        self.linear = nn.Sequential(
            nn.Linear(
                self.hidden_size, intermediate_size, bias=False, dtype=torch.float32
            ),
    ...
    if top_k == 1:
        ...
        if training:
            ...
            mask_for_one_top2 = torch.add(
                0.3333, mask_for_one_top2, alpha=0.6667
            ).type_as(masked_gates_top2)
...
class PhiMoESparseMoeBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.token_frequencies = defaultdict(
            lambda: np.zeros((self.num_experts))
        )  # 记录每个token的激活情况
        ...
        self.cache_capacity = int(
            cache_ratio * self.config.num_local_experts
        )  # 缓存容量
        print(f"Cache capacity: {self.cache_capacity}")
        self.lru_cache_sim = CacheSimulator(
            capacity=self.cache_capacity,
            strategy="lru",
            max_expert_id=self.config.num_local_experts,
        )
        self.fifo_cache_sim = CacheSimulator(
            capacity=self.cache_capacity,
            strategy="fifo",
            max_expert_id=self.config.num_local_experts,
        )
        self.lfu_cache_sim = CacheSimulator(
            capacity=self.cache_capacity,
            strategy="lfu",
            max_expert_id=self.config.num_local_experts,
        )
        self.lru_cache_sim_real = CacheSimulator(
            capacity=self.cache_capacity,
            strategy="lru",
            max_expert_id=self.config.num_local_experts,
        )
        self.fifo_cache_sim_real = CacheSimulator(
            capacity=self.cache_capacity,
            strategy="fifo",
            max_expert_id=self.config.num_local_experts,
        )
        self.lfu_cache_sim_real = CacheSimulator(
            capacity=self.cache_capacity,
            strategy="lfu",
            max_expert_id=self.config.num_local_experts,
        )
    ...
    def forward(
        ...
        if sequence_length == 1:  # 只统计decode阶段
            ...
            for i in range(batch_size * sequence_length):
                ...
                self.token_frequencies[self.total_tokens + 1] = (
                    scores[i].cpu().detach().numpy()
                )
                current_token_activated_experts = (
                    set()
                )  # To avoid double counting for a single token if k > 1

                for k in range(self.top_k):
                    expert_idx = selected_experts[i, k].item()
                    current_token_activated_experts.add(
                        expert_idx
                    )  # Add to set for this token
                ...
                if prediction_for_current_token_scores is not None:
                    ...
                    current_item_prediction_scores = (
                        prediction_for_current_token_scores[i].squeeze(0)
                    )  # Shape [num_local_experts]
                    num_to_predict = min(
                        self.cache_capacity,
                        int(self.config.num_experts_per_tok * self.config.pre_ratio),
                    )  # 预测的专家数量
                    if current_item_prediction_scores.numel() > 0:
                        predicted_expert_indices_for_token = torch.topk(
                            current_item_prediction_scores,
                            k=num_to_predict,
                            dim=-1,
                            sorted=True,
                        )[1]
                        for predicted_expert_idx in predicted_expert_indices_for_token[
                            :num_to_predict
                        ].tolist():
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, router_logits = self.block_sparse_moe(
            hidden_states_input, prediction_for_current_token_scores
        )
        hidden_states = residual + hidden_states
...
class PhiMoEModel(PhiMoEPreTrainedModel):
    ...
    def get_all_lru_cache_stats(self):
        ...
        for layer_idx, layer in enumerate(self.layers):
            if hasattr(layer, "block_sparse_moe") and hasattr(
                layer.block_sparse_moe, "get_lru_cache_stats"
            ):
    ...
    def get_all_fifo_cache_stats(self):
        ...
        for layer_idx, layer in enumerate(self.layers):
            if hasattr(layer, "block_sparse_moe") and hasattr(
                layer.block_sparse_moe, "get_fifo_cache_stats"
            ):
    ...
    def get_all_lfu_cache_stats(self):
        ...
        for layer_idx, layer in enumerate(self.layers):
            if hasattr(layer, "block_sparse_moe") and hasattr(
                layer.block_sparse_moe, "get_lfu_cache_stats"
            ):
    ...
    def get_all_lru_cache_real_stats(self):
        ...
        for layer_idx, layer in enumerate(self.layers):
            if hasattr(layer, "block_sparse_moe") and hasattr(
                layer.block_sparse_moe, "get_lru_cache_real_stats"
            ):
    ...
    def get_all_fifo_cache_real_stats(self):
        ...
        for layer_idx, layer in enumerate(self.layers):
            if hasattr(layer, "block_sparse_moe") and hasattr(
                layer.block_sparse_moe, "get_fifo_cache_real_stats"
            ):
                all_stats[layer_idx] = (
                    layer.block_sparse_moe.get_fifo_cache_real_stats()
                )
    ...
    def get_all_lfu_cache_real_stats(self):
        ...
        for layer_idx, layer in enumerate(self.layers):
            if hasattr(layer, "block_sparse_moe") and hasattr(
                layer.block_sparse_moe, "get_lfu_cache_real_stats"
            ):
    ...
    def forward(
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if sequence_length != 1:
                ...
                prediction_for_current_token_scores = self.previous_predictor_outputs[
                    layer_idx
                ]
        ...
        if self.training:
            ...
            for i, (predictor_output, moe_topk_idx) in enumerate(
                zip(all_predictor_outputs, all_topk_idx)
            ):
                if predictor_output is not None and moe_topk_idx is not None:
                    # 预测器输出 logits，形状为 [batch_size, seq_len, num_local_experts]
                    pred_logits = predictor_output[
                        :, :-predict_dis
                    ]  # 当前 token 预测下一个 token，去掉最后一个
                    # 真实 top-k 索引，形状为 [batch_size * seq_len, top_k]
                    true_topk_idx = moe_topk_idx.view(
                        predictor_output.shape[0], -1, self.config.num_experts_per_tok
                    )[:, predict_dis:]  # 下一个 token 的真实值，去掉第一个
                    batch_size, seq_length = pred_logits.shape[0], pred_logits.shape[1]

                    # 将 true_topk_idx 转换为 one-hot 编码S
                    true_topk_onehot = torch.zeros_like(
                        pred_logits, dtype=torch.float32
                    )
                    for k in range(self.config.num_experts_per_tok):
                        true_topk_onehot.scatter_(
                            2, true_topk_idx[:, :, k].view(batch_size, seq_length, 1), 1
                        )
                    ...
                    loss = loss_fct(
                        pred_logits.reshape(-1, self.config.num_local_experts).float(),
                        true_topk_onehot.reshape(
                            -1, self.config.num_local_experts
                        ).float(),
                    )
                    mask = (
                        ori_attention_mask[:, predict_dis:]
                        .reshape(-1, 1)
                        .expand_as(loss)
                    )  # 去掉第一个 token 的 mask
            ...
            self.training == False
            and all_predictor_outputs[1].shape[0] * all_predictor_outputs[1].shape[1]
            != 1
        ):  # 评测prefill 阶段，但是没有适配decode阶段
            self.iou_scores = []  # 存储每一层的 IoU
            ...
            for i in range(len(self.layers)):
                if (
                    all_predictor_outputs[i] is not None and all_topk_idx[i] is not None
                ):  # 当前层有预测和真实值
                    # 预测器输出 logits，形状为 [batch_size, seq_len, num_local_experts]
                    pred_logits = all_predictor_outputs[i][
                        :, :-predict_dis
                    ]  # 当前 token 预测下一个 token，去掉最后一个
                    true_topk_idx = all_topk_idx[i].view(
                        pred_logits.shape[0], -1, self.config.num_experts_per_tok
                    )[:, predict_dis:]  # 下一 token 的真实值，去掉第一个
                    batch_size, seq_length = pred_logits.shape[0], pred_logits.shape[1]
                    ...
                    mask = (
                        ...
                        .reshape(-1, 1)
                        .expand_as(loss)
                    )  # [batch_size * seq_len - 1, num_local_experts]
                    masked_loss = loss.to(mask.device) * mask  # 屏蔽 <pad> token 的损失
                    loss = masked_loss.sum() / mask.sum()  # 平均有效 token 的损失
                    predictor_losses.append(loss)

                    # 计算 IoU
                    pred_topk = torch.topk(
                        pred_logits, k=self.config.num_experts_per_tok, dim=-1
                    )[1]  # [batch_size, seq_len, top_k]

                    pred_mask = torch.zeros_like(pred_logits).scatter_(
                        2, pred_topk, 1
                    )  # [batch_size, seq_len, num_local_experts]
                    intersection = (pred_mask * true_topk_onehot).sum(
                        dim=-1
                    )  # [batch_size, seq_len]

                    correct = intersection / self.config.num_experts_per_tok
                    masked_accuracy = (
                        correct.float().to(ori_attention_mask.device)
                        * ori_attention_mask[:, predict_dis:]
                    ).sum() / ori_attention_mask[
                        :, predict_dis:
                    ].sum()  # 只计算有效 token 的准确率

                    union = (
                        pred_mask.sum(dim=-1)
                        + true_topk_onehot.sum(dim=-1)
                        - intersection
                    )  # [batch_size, seq_len]
                    iou = intersection / (
                        union + 1e-8
                    )  # 避免除以零，[batch_size, seq_len]
                    masked_iou = (
                        iou.to(ori_attention_mask.device)
                        * ori_attention_mask[:, predict_dis:]
                    ).sum() / ori_attention_mask[
                        :, predict_dis:
                    ].sum()  # 只计算有效 token 的 IoU
