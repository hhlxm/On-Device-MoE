...
class Predictor(nn.Module):
    def __init__(self, config):
        ...
        self.linear = nn.Linear(
            self.hidden_size, config.n_routed_experts, bias=False, dtype=torch.float32
        )
...
class DeepseekV2MoE(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        self.last_topk_idx = topk_idx.detach()  # Store last topk_idx for inference
...
class DeepseekV2DecoderLayer(nn.Module):
    def __init__(self, config: DeepseekV2Config, layer_idx: int):
        ...
        # 添加预测器（仅对 MoE 层）
        if layer_idx < config.num_hidden_layers - config.first_k_dense_replace:
...
class DeepseekV2Model(DeepseekV2PreTrainedModel):
    ...
    def forward(
        ...
        predictor_loss = None
        if self.training and len(all_topk_idx) > self.pre_dis:  # 层数大于1
            predictor_losses = []
            for i in range(len(self.layers) - self.pre_dis):
                if (
                    all_predictor_outputs[i] is not None
                    and all_topk_idx[i + self.pre_dis] is not None
                ):  # 前一层有预测，后self.pre_dis层有真实值
                    ...
                    true_topk_idx = all_topk_idx[i + self.pre_dis]
                    ...
                    loss = loss_fct(
                        pred_logits.view(-1, self.config.n_routed_experts),
                        true_topk_onehot.view(-1, self.config.n_routed_experts),
                    )
                    mask = ori_attention_mask.view(-1, 1).expand_as(
                        loss
            ...
            for i in range(len(self.layers) - self.pre_dis):
                if (
                    ...
                    masked_accuracy = (
                        correct.float() * ori_attention_mask
                    ).sum() / ori_attention_mask.sum()  # 只计算有效 token 的准确率
                    ...
                    masked_iou = (
                        iou * ori_attention_mask
                    ).sum() / ori_attention_mask.sum()  # 只计算有效 token 的 IoU
                    self.iou_scores.append(masked_iou.item())

                    self.accuracy_scores.append(masked_accuracy.item())
                    self.tokens = ori_attention_mask.sum().item()
