...
class DeepseekV2MoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.token_frequencies = defaultdict(
            lambda: np.zeros((self.config.n_routed_experts))
        )  # 记录每个token的激活情况
        ...
        self.cache_capacity = int(
            cache_ratio * self.config.n_routed_experts
        )  # 缓存容量
        ...
        self.lru_cache_sim = CacheSimulator(
            ...
            strategy="lru",
            max_expert_id=self.config.n_routed_experts,
        )
            ...
            for i in range(batch_size * sequence_length):
                ...
                if prediction_for_current_token_scores is not None:
                    ...
                    if current_item_prediction_scores.numel() > 0:
                        ...
                        for predicted_expert_idx in (
                            predicted_expert_indices_for_token[:num_to_predict].tolist()
                        )[::-1]:  # 逆序访问
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if isinstance(decoder_layer.mlp, DeepseekV2MoE):
                ...
                if len(layer_outputs) > (2 if output_attentions else 1) + (
                    ...
                    all_predictor_outputs.append(
                        predictor_output
                    )  # 最后一个输出是 predictor_output
        ...
        if self.training:
            ...
            for i, (predictor_output, moe_topk_idx) in enumerate(
                ...
                if predictor_output is not None and moe_topk_idx is not None:
                    ...
                    loss = loss_fct(
                        pred_logits.reshape(-1, self.config.n_routed_experts),
            ...
            for i in range(len(self.layers)):
                if (
                    ...
                    mask = (
                        ...
                        .reshape(-1, 1)
                        .expand_as(loss)
                    )  # [batch_size * seq_len - 1, n_routed_experts]
                    ...
                    masked_accuracy = (
                        correct.float() * ori_attention_mask[:, predict_dis:]
                    ...
                    masked_iou = (
                        iou * ori_attention_mask[:, predict_dis:]
                    ).sum() / ori_attention_mask[
                        :, predict_dis:
