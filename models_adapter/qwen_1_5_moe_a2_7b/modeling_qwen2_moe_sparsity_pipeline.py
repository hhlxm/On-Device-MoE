...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    def _sparse_expert_compute(
        ...
        normally; only the activation before down_proj is masked.
        """
        ...
        for expert_idx, num_tokens in enumerate(tokens_per_expert):
            ...
            if use_prediction:
                ...
                if missing.any():
                    ...
                    if ondemand:
                        _, sparse_idx = torch.topk(up_out[missing_rows].abs(), n_keep, dim=-1, sorted=False)
                        mask[missing_rows[:, None], sparse_idx] = 1.0
                    else:
                        mask[missing_rows] = 1.0
            else:
                _, topk_indices = torch.topk(up_out.abs(), n_keep, dim=-1, sorted=False)
                mask = torch.zeros_like(act)
    ...
    def _predict_next_layer(
        ...
        for expert_idx, num_tokens in enumerate(tokens_per_expert):
            ...
            expert_input = sorted_tokens[start_idx:end_idx].to(
                device=expert.up_proj.weight.device, dtype=expert.up_proj.weight.dtype
            )
            up_pred = expert.up_proj(expert_input)
            _, sparse_idx = torch.topk(up_pred.abs(), n_keep, dim=-1, sorted=False)
