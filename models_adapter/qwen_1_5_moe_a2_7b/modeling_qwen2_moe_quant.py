...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.Tensor, quant_wbit: int = 0, quant_num: int = 0
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """ """
        ...
        if (
            not self.training
            and quant_num == self.top_k
            and not self.all_quant
            and routing_weights.shape[0] == 1
        ):
            ...
            for expert_idx, expert in enumerate(self.experts):
                try:
                    ...
                    logging.error(
                        f"Full quantization failed for expert {expert_idx}: {e}"
                    )
                    raise
            self.all_quant = True
        ...
        if (
            ...
            assert 0 <= quant_num <= self.top_k, (
                f"quant_num must be in [0, {self.top_k}]"
            )
            # 选择最低 quant_num 个专家
            ...
            if bottom_experts.numel() == 0:
                ...
                for expert_idx in bottom_experts.view(-1).unique():
                    ...
                    assert 0 <= expert_idx < self.num_experts, (
                        f"Invalid expert_idx: {expert_idx}"
                    )
                    ...
                    try:
                        for attr in weight_attrs:
                            ...
                            getattr(expert, attr).weight.data = self.quant_cache[
                                cache_key
                            ]
                            # print(cache_key,"success")
                    except Exception as e:
                        logging.error(
                            f"Quantization failed for expert {expert_idx}: {e}"
                        )
                        raise
            ...
            for expert_idx in quant_experts:
                ...
                for attr in weight_attrs:
                    ...
                    if cache_key in self.original_weights:
                        getattr(expert, attr).weight.data = self.original_weights[
                            cache_key
                        ]
