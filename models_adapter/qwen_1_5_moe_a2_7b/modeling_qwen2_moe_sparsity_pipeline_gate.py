...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    @torch.no_grad()
    def _use_predicted_routing(self, prediction_state):
        """Pure prefetch mode: use predicted next-layer routing probabilities."""
        probs = prediction_state["predicted_probs"]
        routing_weights, selected_experts = torch.topk(probs, self.top_k, dim=-1, sorted=False)
        if self.norm_topk_prob:
            routing_weights /= routing_weights.sum(dim=-1, keepdim=True)
        return selected_experts, routing_weights

    @torch.no_grad()
    def _sparse_expert_compute(
        self,
        x,
        topk_ids,
        topk_weight,
        neural_sparsity_ratio,
        prediction_state,
        prefetch,
        ondemand,
    ):
        """
        Sparse expert computation for decode. Gate/up projections are computed
        normally; sparse neuron indices are selected from gate_proj output and
        only the activation before down_proj is masked.
        """
        ...
        sorted_assignment_idx = topk_flat.argsort()
        token_indices = sorted_assignment_idx // topk_ids.shape[1]
        sorted_tokens = x[token_indices]
        tokens_per_expert = torch.bincount(topk_flat, minlength=self.num_experts).cpu().tolist()
        ...
        for expert_idx, num_tokens in enumerate(tokens_per_expert):
            ...
            current_state = sorted_tokens[start_idx:end_idx]
            ...
            if use_prediction:
                current_token_idx = token_indices[start_idx:end_idx]
                lookup_token_idx = current_token_idx.to(pred_experts.device)
                current_pred_experts = pred_experts.index_select(0, lookup_token_idx)
                matches = current_pred_experts.eq(expert_idx)
                has_match = matches.any(dim=1)
                mask = torch.zeros_like(act)

                if has_match.any():
                    rows_pred = has_match.nonzero(as_tuple=True)[0]
                    rows = rows_pred.to(act.device)
                    slots = matches.to(torch.int64).argmax(dim=1)
                    sparse_idx = pred_sparse[
                        lookup_token_idx[rows_pred], slots[rows_pred].to(pred_sparse.device)
                    ].to(act.device)
                    mask[rows[:, None], sparse_idx] = 1.0

                missing = ~has_match
                if missing.any():
                    missing_rows = missing.nonzero(as_tuple=True)[0].to(act.device)
                    if ondemand:
                        _, sparse_idx = torch.topk(gate_out[missing_rows].abs(), n_keep, dim=-1, sorted=False)
                        mask[missing_rows[:, None], sparse_idx] = 1.0
                    else:
                        mask[missing_rows] = 1.0
            else:
                _, topk_indices = torch.topk(gate_out.abs(), n_keep, dim=-1, sorted=False)
                mask = torch.zeros_like(act)
                mask.scatter_(-1, topk_indices, 1.0)

            outputs.append(expert.down_proj(act * mask))
            start_idx = end_idx

        outs = torch.cat(outputs, dim=0) if outputs else sorted_tokens.new_empty((0, hidden_dim))
        restored = torch.empty_like(outs)
        restored[sorted_assignment_idx] = outs
        final_hidden_states = (
    ...
    def _predict_next_layer(
        ...
        """
        Predict the next MoE layer's experts and sparse neuron indices from
        the current layer's normalized FFN input.
        """
        n_tokens = ffn_input.shape[0]
        intermediate_size = next_moe_block.config.moe_intermediate_size
        n_keep = int(intermediate_size * (1 - neural_sparsity_ratio))

        gate_weight = next_moe_block.gate.weight
        gate_bias = next_moe_block.gate.bias
        gate_input = ffn_input.to(device=gate_weight.device, dtype=gate_weight.dtype)
        router_logits = F.linear(gate_input, gate_weight, gate_bias)
        scores = F.softmax(router_logits, dim=-1, dtype=torch.float32)

        n_pred = min(n_expert_used, max(1, int(n_expert_used * prefetch_expert_ratio)))
        pred_weight, pred_idx = torch.topk(scores, n_pred, dim=-1, sorted=False)

        pred_sparse = torch.zeros(n_tokens, n_pred, n_keep, dtype=torch.long, device=scores.device)
        pred_flat = pred_idx.reshape(-1)
        sorted_assignment_idx = pred_flat.argsort()
        sorted_tokens = gate_input[sorted_assignment_idx // n_pred]
        tokens_per_expert = torch.bincount(pred_flat, minlength=next_moe_block.num_experts).cpu().tolist()
        pred_sparse_flat = pred_sparse.view(-1, n_keep)
        ...
        for expert_idx, num_tokens in enumerate(tokens_per_expert):
            ...
            expert = next_moe_block.experts[expert_idx]
            expert_input = sorted_tokens[start_idx:end_idx].to(
                device=expert.gate_proj.weight.device, dtype=expert.gate_proj.weight.dtype
            )
            gate_pred = expert.gate_proj(expert_input)
            _, sparse_idx = torch.topk(gate_pred.abs(), n_keep, dim=-1, sorted=False)
            pred_sparse_flat[sorted_assignment_idx[start_idx:end_idx]] = sparse_idx.to(pred_sparse.device)
            start_idx = end_idx

        return {
            "predicted_probs": scores,
            "predicted_expert_idx": pred_idx,
            "predicted_expert_weights": pred_weight,
            "predicted_sparse_indices": pred_sparse,
        }
    ...
    def forward_sparse(
        ...
        prediction_state,
        next_moe_block,
        prefetch_expert_ratio,
        is_first_moe,
    ):
        """
        Sparse MoE forward for decode phase.
        Returns: (output, router_logits, new_prediction_state)
        """
        batch_size, sequence_length, hidden_dim = hidden_states.shape
        x = hidden_states.view(-1, hidden_dim)

        use_predicted_routing = prefetch and not ondemand and not is_first_moe and prediction_state is not None
        if use_predicted_routing:
            selected_experts, routing_weights = self._use_predicted_routing(prediction_state)
            selected_experts = selected_experts.to(x.device)
            routing_weights = routing_weights.to(device=x.device, dtype=x.dtype)
            router_logits = None
        else:
        ...
        if is_first_moe:
            y, first_router_logits = self.forward(hidden_states)
            if router_logits is None:
                router_logits = first_router_logits
        else:
        ...
        new_prediction_state = None
        if prefetch and next_moe_block is not None:
            ep_ratio = prefetch_expert_ratio if ondemand else 1.0
            new_prediction_state = self._predict_next_layer(
                x,
                next_moe_block,
                neural_sparsity_ratio,
                self.top_k,
        ...
        return y, router_logits, new_prediction_state
...
class Qwen2MoeDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2MoeConfig, layer_idx: int):
        ...
        self.is_moe_layer = (layer_idx not in config.mlp_only_layers) and (
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,  # will become mandatory in v4.46
        prediction_state: Optional[dict] = None,
        ...
        **kwargs,
        ...
        if self.is_moe_layer and sparsity_config is not None:
            next_moe = sparsity_config.get("next_moe_map", {}).get(self.layer_idx)
            is_first = sparsity_config.get("first_moe_idx") == self.layer_idx
            hidden_states, router_logits, new_prediction_state = self.mlp.forward_sparse(
                hidden_states,
                neural_sparsity_ratio=sparsity_config["neural_sparsity_ratio"],
                prefetch=sparsity_config["prefetch"],
                ondemand=sparsity_config["ondemand"],
                prediction_state=prediction_state,
                next_moe_block=next_moe,
                prefetch_expert_ratio=sparsity_config.get("prefetch_expert_ratio", 1.0),
...
class Qwen2MoeModel(Qwen2MoePreTrainedModel):
    ...
    def __init__(self, config: Qwen2MoeConfig):
        ...
        self.moe_layer_indices = [i for i, layer in enumerate(self.layers) if layer.is_moe_layer]
        self.next_moe_map = {}
        for i, idx in enumerate(self.moe_layer_indices):
            if i + 1 < len(self.moe_layer_indices):
                next_idx = self.moe_layer_indices[i + 1]
                self.next_moe_map[idx] = self.layers[next_idx].mlp
        self.first_moe_idx = self.moe_layer_indices[0] if self.moe_layer_indices else None
    ...
    def forward(
        ...
        is_decode = inputs_embeds.shape[1] == 1 and not self.training and neural_sparsity_ratio > 0.0
        if is_decode:
            sparsity_config = {
                "neural_sparsity_ratio": neural_sparsity_ratio,
                "prefetch": prefetch,
                "ondemand": ondemand,
                "prefetch_expert_ratio": prefetch_expert_ratio,
                "next_moe_map": self.next_moe_map,
                "first_moe_idx": self.first_moe_idx,
            }
        ...
        for decoder_layer in self.layers:
            ...
            if sparsity_config is not None:
                prediction_state = layer_outputs[-1]
                if output_router_logits:
                    router_idx = 3 if output_attentions and use_cache else 2 if output_attentions or use_cache else 1
                    router_logits = layer_outputs[router_idx] if len(layer_outputs) > router_idx else None
