...
class Predictor(nn.Module):
    def __init__(self, config):
        ...
        intermediate_size = int((self.hidden_size + config.num_experts) * 5 / 3)
        self.linear = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(
                intermediate_size, config.num_experts, bias=False, dtype=torch.float32
            ),
        )
...
class Qwen2MoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        self.cache_capacity = int(cache_ratio * self.config.num_experts)  # 缓存容量
        print(f"Cache capacity: {self.cache_capacity}")
        self.lru_cache_sim = CacheSimulator(
            capacity=self.cache_capacity,
            strategy="lru",
            max_expert_id=self.config.num_experts,
        )
    ...
    def forward(
        ...
        if sequence_length == 1:  # 只统计decode阶段
            ...
            for i in range(batch_size * sequence_length):
                scores = router_logits.softmax(dim=-1, dtype=torch.float)
                ...
                if prediction_for_current_token_scores is not None:
                    # prediction_for_current_token_scores has shape [bsz, 1, num_experts]
...
class Qwen2MoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.mlp(hidden_states, prediction_for_current_token_scores)
...
class Qwen2MoeModel(Qwen2MoePreTrainedModel):
    ...
    def get_all_fifo_cache_real_stats(self):
        ...
        for layer_idx, layer in enumerate(self.layers):
            if hasattr(layer, "mlp") and hasattr(
                layer.mlp, "get_fifo_cache_real_stats"
            ):
    ...
    def get_all_expert_frequencies(self):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            # print(f'sum:{moe_block.total_tokens_nor}')
        ...
        if self.training:
            ...
            for i, (predictor_output, moe_topk_idx) in enumerate(
                ...
                if predictor_output is not None and moe_topk_idx is not None:
                    ...
                    loss = loss_fct(
                        pred_logits.reshape(-1, self.config.num_experts).float(),
                        true_topk_onehot.reshape(-1, self.config.num_experts).float(),
                    )
                    mask = (
