...
class OlmoeSparseMoeBlock(nn.Module):
    ...
    # ------------------------------------------------------------------ #
    #  Neuron-sparsity pipeline (decode-only, all weights on GPU)         #
    # ------------------------------------------------------------------ #

    @torch.no_grad()
    def _use_predicted_routing(self, prediction_state):
        """Pure prefetch: derive routing from predicted probs."""
        ...
        if self.norm_topk_prob:
            routing_weights /= routing_weights.sum(dim=-1, keepdim=True)
        routing_weights = routing_weights.to(probs.dtype)
        ...
        for tok in range(n_tokens):
            for slot in range(n_expert_used):
                ...
                if sparse_idx is None and ondemand:
                    ...
                    _, sparse_idx = torch.topk(gate_out.abs().squeeze(0), n_keep)
    ...
    def _predict_next_layer(self, ffn_input, next_moe_block,
        ...
        """
        Predict next layer's expert activation and neuron sparsity.
        ffn_input:      [n_tokens, hidden_size]
        next_moe_block: OlmoeSparseMoeBlock
        """
    ...
    def forward_sparse(self, hidden_states, neural_sparsity_ratio,
                       ...
                       next_moe_block, prefetch_expert_ratio,
                       is_first_moe):
        """
        Sparse MoE forward for decode (all weights on GPU).
        ...
        # --- routing ---
        use_pred = (prefetch and not ondemand and not is_first_moe
                    and prediction_state is not None)
        if use_pred:
            selected_experts, routing_weights = self._use_predicted_routing(
                prediction_state)
            router_logits = None
        else:
        ...
        if is_first_moe:
            ...
            y = self._sparse_expert_compute(
                ...
                neural_sparsity_ratio, prediction_state, prefetch, ondemand
        ...
        if prefetch and next_moe_block is not None:
            ...
            new_prediction_state = self._predict_next_layer(
                x, next_moe_block, neural_sparsity_ratio,
                self.top_k, ep_ratio)
...
class OlmoeDecoderLayer(nn.Module):
    def __init__(self, config: OlmoeConfig, layer_idx: int):
        ...
        self.hidden_size = config.hidden_size
        self.layer_idx = layer_idx
        self.is_moe_layer = True  # every OLMoE layer is MoE
    ...
    def forward(
        ...
        **kwargs,
        ...
        if sparsity_config is not None:
            ...
            hidden_states, router_logits, new_prediction_state = \
                self.mlp.forward_sparse(
                    hidden_states,
...
class OlmoeModel(OlmoePreTrainedModel):
    ...
    def __init__(self, config: OlmoeConfig):
        ...
        # Build MoE layer mapping (every layer is MoE in OLMoE)
        self.moe_layer_indices = list(range(config.num_hidden_layers))
        self.next_moe_map = {}
        for i in range(config.num_hidden_layers - 1):
            self.next_moe_map[i] = self.layers[i + 1].mlp
        self.first_moe_idx = 0
    ...
    def forward(
        ...
        for decoder_layer in self.layers[: self.config.num_hidden_layers]:
            ...
            # When sparsity is active, prediction_state is appended last
            if sparsity_config is not None:
                prediction_state = layer_outputs[-1]
                # router_logits is second-to-last when both are present
