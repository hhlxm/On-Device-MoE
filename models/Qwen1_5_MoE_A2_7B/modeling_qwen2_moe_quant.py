...
class Qwen2MoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        # 量化支持
        self.all_quant = False
        self.quant_cache = {}  # 缓存量化权重
        self.original_weights = {}  # 备份原始权重

    def forward(self, hidden_states: torch.Tensor, quant_wbit: int = 0, quant_num: int = 0) -> tuple[torch.Tensor, torch.Tensor]:
        """ """
        ...
        # 全量化（quant_num == top_k）
        if not self.training and quant_num == self.top_k and not self.all_quant and routing_weights.shape[0] == 1:
            # print("Full quantization")
            weight_attrs = ['gate_proj', 'up_proj', 'down_proj']
            for expert_idx, expert in enumerate(self.experts):
                try:
                    for attr in weight_attrs:
                        weight = getattr(expert, attr).weight.data
                        getattr(expert, attr).weight.data = normal_quantize(
                            w=weight, blocksize=128, wbit=quant_wbit
                        )
                except Exception as e:
                    logging.error(f"Full quantization failed for expert {expert_idx}: {e}")
                    raise
            self.all_quant = True

        # 部分量化（仅 decode 阶段，routing_weights.shape[0] == 1）
        quant_experts = []
        if not self.training and quant_num != 0 and quant_num != self.top_k and routing_weights.shape[0] == 1:
            # print("Partial quantization")
            # print(f"Quantization num: {quant_num}")
            # print(f"Quantization wbit: {quant_wbit}")
            # print(routing_weights.shape)
            assert 0 <= quant_num <= self.top_k, f"quant_num must be in [0, {self.top_k}]"
            # 选择最低 quant_num 个专家
            _, bottom_idx = torch.topk(
                routing_weights, k=quant_num, dim=1, largest=False
            )  # 形状 [1, quant_num]
            bottom_experts = selected_experts.gather(
                dim=1, index=bottom_idx
            )  # 最低 quant_num 个专家的索引
            if bottom_experts.numel() == 0:
                logging.warning("No experts selected for quantization")
            else:
                weight_attrs = ['gate_proj', 'up_proj', 'down_proj']
                for expert_idx in bottom_experts.view(-1).unique():
                    expert_idx = int(expert_idx.item())
                    assert 0 <= expert_idx < self.num_experts, f"Invalid expert_idx: {expert_idx}"
                    expert = self.experts[expert_idx]
                    try:
                        for attr in weight_attrs:
                            weight = getattr(expert, attr).weight.data
                            cache_key = f"expert_{expert_idx}_{attr}_{quant_wbit}"
                            if cache_key not in self.original_weights:
                                self.original_weights[cache_key] = weight.clone()
                            if cache_key not in self.quant_cache:
                                self.quant_cache[cache_key] = normal_quantize(
                                    w=weight, blocksize=128, wbit=quant_wbit
                                )
                            getattr(expert, attr).weight.data = self.quant_cache[cache_key]
                            # print(cache_key,"success")
                    except Exception as e:
                        logging.error(f"Quantization failed for expert {expert_idx}: {e}")
                        raise
                    quant_experts.append(expert_idx)
        ...
        if not self.training and quant_num != 0 and quant_num != self.top_k and routing_weights.shape[0] == 1:
            weight_attrs = ['gate_proj', 'up_proj', 'down_proj']
            for expert_idx in quant_experts:
                expert = self.experts[expert_idx]
                for attr in weight_attrs:
                    cache_key = f"expert_{expert_idx}_{attr}_{quant_wbit}"
                    if cache_key in self.original_weights:
                        getattr(expert, attr).weight.data = self.original_weights[cache_key]
...
class Qwen2MoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,  # necessary, but kept here for BC
        quant_wbit: Optional[int] = None,
        quant_num: Optional[int] = None,
        **kwargs,
        ...
        hidden_states = self.mlp(hidden_states,quant_wbit,quant_num)
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    cache_position=cache_position,
                    position_embeddings=position_embeddings,
                    quant_wbit=self.quant_wbit,
                    quant_num=self.quant_num,
                )
