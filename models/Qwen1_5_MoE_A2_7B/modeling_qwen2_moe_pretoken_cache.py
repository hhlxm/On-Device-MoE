...
class Predictor(nn.Module):
    def __init__(self, config):
        ...
        self.linear = nn.Sequential(
            ...
            nn.ReLU(),  
            nn.Linear(intermediate_size, config.num_experts, bias=False, dtype=torch.float32)
        )
...
class Qwen2MoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        self.lru_cache_sim = CacheSimulator(capacity=self.cache_capacity, strategy='lru',max_expert_id=self.config.num_experts)
        self.fifo_cache_sim = CacheSimulator(capacity=self.cache_capacity, strategy='fifo',max_expert_id=self.config.num_experts)
        self.lfu_cache_sim = CacheSimulator(capacity=self.cache_capacity, strategy='lfu',max_expert_id=self.config.num_experts)
        self.lru_cache_sim_real = CacheSimulator(capacity=self.cache_capacity, strategy='lru',max_expert_id=self.config.num_experts)
        self.fifo_cache_sim_real = CacheSimulator(capacity=self.cache_capacity, strategy='fifo',max_expert_id=self.config.num_experts)
        self.lfu_cache_sim_real = CacheSimulator(capacity=self.cache_capacity, strategy='lfu',max_expert_id=self.config.num_experts)
        ...
        if(sequence_length==1):#只统计decode阶段
            ...
            for i in range(batch_size * sequence_length):
                ...
                if prediction_for_current_token_scores is not None:
                    ...
                    current_item_prediction_scores = prediction_for_current_token_scores[i].squeeze(0) # Shape [num_experts]
        ...
        if self.training:
            ...
            for i, (predictor_output, moe_topk_idx) in enumerate(zip(all_predictor_outputs, all_topk_idx)):
                if predictor_output is not None and moe_topk_idx is not None:
                    ...
                    loss = loss_fct(pred_logits.reshape(-1, self.config.num_experts).float(), 
