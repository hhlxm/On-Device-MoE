...
class CacheSimulator:
    def __init__(self, capacity, strategy='lru'):
        ...
        self.cache = {} # For LFU: expert_id -> CacheEntry object
        self.hits = 0
        self.misses = 0
        self.requests = 0
    ...
    def access(self, expert_id):
        ...
        if self.strategy == 'lru':
            ...
            if expert_id in self.cache_store:
                ...
                # To update in heap: remove old, update, add new. This is inefficient.
                # A common optimization is to mark old entry as invalid and add new,
                # or use a heap that supports decrease-key/increase-key.
                # For simplicity here, we'll rebuild part of it or accept some inefficiency.
                # A more robust LFU heap would require a custom heap or more complex logic.
                
                # Find and remove old entry from heap (this is the inefficient part)
                # This linear scan is bad for performance with large heaps.
                # In a production LFU, you'd use a heap that supports efficient updates or a different structure.
                old_heap_entry = None
...
class DeepseekV2MoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.lru_cache_sim = CacheSimulator(capacity=self.cache_capacity, strategy='lru')
        self.fifo_cache_sim = CacheSimulator(capacity=self.cache_capacity, strategy='fifo')
        self.lfu_cache_sim = CacheSimulator(capacity=self.cache_capacity, strategy='lfu')
        ...
        if(sequence_length==1):#只统计decode阶段
            ...
            for i in range(batch_size * sequence_length):
                ...
                for k in range(self.config.num_experts_per_tok):
                    ...
                    ##使用onehot进行similarity计算
                    # self.token_frequencies[self.total_tokens][expert_idx] = topk_weight[i,k].item()
                    # expert_indices = topk_idx[i].cpu().numpy()  # 转换为 NumPy 数组
                    # # 创建 one-hot 编码
                    # one_hot = np.zeros(self.config.n_routed_experts)
