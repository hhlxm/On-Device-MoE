...
class DeepseekV2Model(DeepseekV2PreTrainedModel):
    ...
    def forward(
        ...
        predictor_loss = None
        if self.training and len(all_topk_idx) > 1:#层数大于1
            predictor_losses = []
            for i in range(len(self.layers) - 1):
                if all_predictor_outputs[i] is not None and all_topk_idx[i + 1] is not None: #前一层有预测，后一层有真实值
                    ...
                    true_topk_idx = all_topk_idx[i + 1]
                    ...
                    # print(f"pred_logits: {pred_logits.min().item()}, {pred_logits.max().item()}")
                    # print(f"true_topk_onehot: {true_topk_onehot.min().item()}, {true_topk_onehot.max().item()}")
                    # print(f"pred_logits shape: {len(pred_logits)}")
                    # print(f"true_topk_onehot shape: {len(true_topk_onehot)}")
            ...
            print(f'all_predictor_outputs shape:{len(all_predictor_outputs)}')
            print(f'all_topk_idx shape:{len(all_topk_idx)}')
            self.iou_scores = []  # 存储每一层的 IoU
            self.accuracy_scores = []
            predictor_losses = []
            for i in range(len(self.layers) - 1):
                if all_predictor_outputs[i] is not None and all_topk_idx[i + 1] is not None:  # 前一层有预测，后一层有真实值
                    ...
                    self.iou_scores.append(iou.mean().item())  # 记录每层的平均 IoU
                    ...
                    accuracy = correct.float().mean().item()  # 计算平均准确率
                    self.accuracy_scores.append(accuracy)  # 记录每层的准确率
                    self.tokens = batch_size * seq_length  # 累加总的 token 数量
