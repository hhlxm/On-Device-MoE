...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        attention_mask = [1] * len(a_ids + b_ids + [self.tokenizer.eos_token_id]) + [
            0
        ] * pad_len
    ...
    for epoch in range(train_config["num_epochs"]):
        for step, batch in enumerate(train_dataloader):
            ...
            if global_step % train_config["eval_every"] == 0:
                ...
                os.makedirs(save_dir, exist_ok=True)
                plot_losses(
                    train_loss, val_loss, os.path.join(save_dir, "loss_plot.svg")
                )
                plot_metrics(val_iou, "iou", os.path.join(save_dir, "val_iou_plot.svg"))
                plot_metrics(
                    val_acc, "accuracy", os.path.join(save_dir, "val_acc_plot.svg")
                )
