...
def save_model(model, train_config, save_dir):
    ...
    predictor_state_dict = {
        k: v for k, v in model.state_dict().items() if "predictor" in k
    }
    ...
    save_path = os.path.join(
        save_dir, f"checkpoint_epoch_{train_config['num_epochs']}.pt"
    )
...
def validate(model, val_dataloader, max_steps=2):
    ...
    avg_iou = (
        [
            sum(step_iou[i] for step_iou in v_iou) / val_step
            for i in range(len(v_iou[0]))
        ]
        if v_iou
        else []
    )
    avg_acc = (
        [
            sum(v_acc[s][i] * v_tokens[s] for s in range(val_step)) / sum(v_tokens)
            for i in range(len(v_acc[0]))
        ]
        if v_acc and v_tokens
        else []
    )
    ...
    for epoch in range(train_config["num_epochs"]):
        for step, batch in enumerate(train_dataloader):
            ...
            if global_step % train_config["eval_every"] == 0:
                v_loss, v_iou, v_acc = validate(
                    model,
                    val_dataloader,
                    max_steps=train_config.get("val_max_steps", 2),
                )
                ...
                os.makedirs(save_dir, exist_ok=True)
                plot_losses(
                    train_loss, val_loss, os.path.join(save_dir, "loss_plot.png")
                )
                plot_losses(
                    {
                        key: np.mean(values) if values else None
                        for key, values in val_acc.items()
                    },
                    {
                        key: np.mean(values) if values else None
                        for key, values in val_iou.items()
                    },
                    os.path.join(save_dir, "val_acc.png"),
                )
                plot_metrics(val_iou, "iou", os.path.join(save_dir, "val_iou_plot.png"))
                plot_metrics(
                    val_acc, "accuracy", os.path.join(save_dir, "val_acc_plot.png")
                )
            ...
            if (step + 1) % train_config["print_every"] == 0:
                ...
                logging.info(
                    f"Epoch {epoch + 1}/{train_config['num_epochs']}, Step {step + 1}/{len(train_dataloader)}, "
...
def main():
    ...
    config_path = (
        sys.argv[1]
        if len(sys.argv) > 1
        else "/home/fit/renju/WORK/lxm/train_config.yaml"
    )
    ...
    # 设置日志
    setup_logging(save_dir, data_config["test_dataset_name"])
    ...
    config_save_path = os.path.join(
        ...
        f"config_epoch_{train_config['num_epochs']}.yaml",
    )
    ...
    if (not train_config["num_epochs"] == 0) and (
        train_config["train_max_steps"] == -1
    ):
        train_config["train_max_steps"] = (
        ...
        if model_config.get("type", "pregate") in ["pretoken", "pregate", "prefetch"]:
            ...
            for (
                layer
            ) in model.model.layers:  # 假设 decoder 层位于 model.model.layers 中
                if layer.predictor is not None:
                    nn.init.kaiming_uniform_(
                        layer.predictor.linear.weight, a=math.sqrt(5)
                    )
        ...
        v_loss, v_iou, v_acc = validate(
            model, val_dataloader, max_steps=train_config.get("val_max_steps", 2)
        )
        plot_metrics(
            v_iou,
            "iou",
