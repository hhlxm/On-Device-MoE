...
from Analyse.my_dataload import CustomDataset, instruct_CustomDataset
...
def load_model_and_tokenizer(model_config):
    ...
    if type not in ["pregate", "pretoken"]:
        raise ValueError(
            "Invalid type. Choose from 'pregate', 'finetune', or 'pretoken'."
        )
    ...
    if type == "pregate":
        from models.DeepSeek_V2_Lite.modeling_deepseek_pregate_predictor import (
            DeepseekV2ForCausalLM,
        )
        from models.Phi_3_5_MoE_instruct.modeling_phimoe_pregate_test import (
            PhiMoEForCausalLM,
        )
        from models.Qwen1_5_MoE_A2_7B.modeling_qwen2_moe_pregate_test import (
            Qwen2MoeForCausalLM,
        )
        from models.Mixtral_8x7B_v0_1.modeling_mixtral_pregate_test import (
            MixtralForCausalLM,
        )
    elif type == "pretoken":
        from models.DeepSeek_V2_Lite.modeling_deepseek_pretoken_predictor import (
            DeepseekV2ForCausalLM,
        )
        from models.Phi_3_5_MoE_instruct.modeling_phimoe_pretoken import (
            PhiMoEForCausalLM,
        )
        from models.Qwen1_5_MoE_A2_7B.modeling_qwen2_moe_pretoken import (
            Qwen2MoeForCausalLM,
        )
        from models.Mixtral_8x7B_v0_1.modeling_mixtral_pretoken import (
            MixtralForCausalLM,
        )
    # elif type == 'prefetch':
    ...
    if type == "pregate":
        model.model.pre_dis = model_config.get(
            "pre_dis", 1 if "deepseek" in model_config["name"].lower() else 0
        )
        model.model.pre_ahead = model_config.get("pre_ahead", 1)
        print(
            f"Model loaded from {model_config['name']} with type {type} Pre_dis:{model.model.pre_dis}  Pre_ahead:{model.model.pre_ahead}"
        )
...
def load_data(tokenizer, data_config):
    if "alpaca" in data_config["train_dataset_name"].lower():
        print("Loading Alpaca dataset...")
        train_dataset = instruct_CustomDataset(
            data_config["train_dataset_name"],
            data_config["train_type"],
            tokenizer,
            max_length=data_config.get("max_length", 512),
        )
        val_dataset = instruct_CustomDataset(
            data_config["test_dataset_name"],
            data_config["test_type"],
            tokenizer,
            max_length=data_config.get("max_length", 512),
        )
    else:
